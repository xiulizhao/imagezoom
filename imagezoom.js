(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.imagezoom) {
      window.wwload.imagezoom = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/imagezoom/common","css!libs/imagezoom/common"], function() {
        window.wwload.imagezoom = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.imagezoom === "wait") {
      setTimeout(function () {
   ...
   */
  function images_zoom($ele){
    var config = {};
    config.boxw = eval($ele.attr("data-boxw") || "400");
    config.boxh = eval($ele.attr("data-boxh") || "400");
    config.sumw = eval($ele.attr("data-sumw") || "50");
    config.sumh = eval($ele.attr("data-sumh") || "50");
    config.sumi = eval($ele.attr("data-sumi") || "7");
    config.sums = eval($ele.attr("data-sums") || "6");
    config.sumborder = eval($ele.attr("data-sumborder") || "1");

    $(document).ready(function(){
  	  var showproduct = {
  		  "boxid":"showbox",
  		  "sumid":"showsum",
        "boxw": config.boxw,                   //宽度,该版本中请把宽高填写成一样
        "boxh": config.boxh,                   //高度,该版本中请把宽高填写成一样
        "sumw": config.sumw,                    //列表每个宽度,该版本中请把宽高填写成一样
        "sumh": config.sumh,                    //列表每个高度,该版本中请把宽高填写成一样
        "sumi": config.sumi,                     //列表间隔
        "sums": config.sums,                     //列表显示个数
  		  "sumsel":"sel",
        "sumborder": config.sumborder,                //列表边框，没有边框填写0，边框在css中修改
  		  "lastid":"showlast",
  		  "nextid":"shownext"
  		  };//参数定义
  ...
  function processElement($ele) {
    ...
    setTimeout(function () {
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
      $ele.find(".imagezoom_content").removeClass("hidden");
      images_zoom($ele);
    }, 1000);
